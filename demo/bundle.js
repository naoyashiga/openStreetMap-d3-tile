...
var locations = d3.range(locationsCount).map(function () {
  ...
  var wiggleScale = 0.01;
  var lng = location.longitude + wiggle(wiggleScale);
  var lat = location.latitude + wiggle(wiggleScale);
...
var Viz = function () {
  function Viz() {
    ...
    this.tile = Tile.tile().size([this.width, this.height]);
  ...
  _createClass(Viz, [{
    ...
    value: function loadData() {
      var center = this.projection([location.longitude, location.latitude]);

      this.vector.datum({ type: "FeatureCollection", features: locations });

      this.svg.call(this.zoom).call(this.zoom.transform, d3.zoomIdentity.translate(this.width / 2, this.height / 2).scale(1 << 22).translate(-center[0], -center[1]));
    }
  }, {
    key: "zoomed",
    ...
    key: "updateTile",
    value: function updateTile(transform) {
      var tiles = this.tile.scale(transform.k).translate([transform.x, transform.y])();

      var image = this.raster.attr('transform', stringify(tiles.scale, tiles.translate)).selectAll('image').data(tiles, function (d) {
        return d;
      });
      ...
      image.enter().append('image').attr("xlink:href", function (d) {
        ...
        return d[0] * imageSize;
      }).attr("y", function (d) {
        return d[1] * imageSize;
      }).attr("width", imageSize).attr("height", imageSize);

      this.projection.scale(transform.k / tau).translate([transform.x, transform.y]);

      this.vector.attr('d', this.path);
    }
  }, {
    key: "createProjection",
    value: function createProjection() {
      return d3.geoMercator().scale(1 / tau).translate([0, 0]);
    }
  }, {
    key: "createScreen",
    value: function createScreen() {
      return d3.select('body').append('svg').attr('class', 'map').attr('width', this.width).attr('height', this.height);
    }
  }, {
    key: "resizeWindow",
    value: function resizeWindow() {
      ...
      d3.selectAll(".map").attr('width', this.width).attr('height', this.height);
...
},{"d3":4,"d3-tile":3}],2:[function(require,module,exports){
...
},{"d3-array":2}],4:[function(require,module,exports){
